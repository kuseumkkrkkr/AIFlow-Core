...
2027학년도 6월 수학 영역의 로컬 점검에서는 원문 해시를 붙인 문항을 `private_benchmarks/official/kice_2027_06/corpus.json`에만 추가한다. 이 코퍼스에서 확인된 `a^(mx+p)=b^(nx+q)`, 이차다항식 차분몫, 원점 동시 출발에서 `v₁(t)=at²+bt`, `v₂(t)=ct`인 두 점의 위치 일치 시각은 실행 계약으로 승격한다. 마지막 계약은 `t=3(c-b)/(2a)>0` 및 위치 차 재대입을 모두 요구한다. 그림 의존 극한, 일반 삼각·로그 식은 성공값을 추정하지 않고 `FAIL`로 남긴다. 보고서의 허위 `PASS`율은 이 미지원 문항까지 포함해 계산한다.
