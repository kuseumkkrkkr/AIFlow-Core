...
생성 문항 통과율은 생성기가 만든 지원 범위의 안정성만 말한다. 실제 문항 일반화는 코퍼스에서 `expected`와 엔진의 `answer`를 비교해 확인한다. `benchmarks/official_exam_regression.json`은 저작권을 고려해 2026년 6월 고2 30번의 원문 전체가 아닌 조건 구조를 보존한 회귀 입력이다.
