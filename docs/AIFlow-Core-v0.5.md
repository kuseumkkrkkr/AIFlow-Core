...
현재 실행 규칙은 일차·이차방정식, 비율, 집합, 확률·조합·순열, 기본 도형, 등차·등비수열, 함수값·합성함수·역함수, 인수분해·나머지정리, 조건부확률, 지수·로그와 지수·로그 방정식, 특수각 삼각함수, 다항식 극한, 거듭제곱 미분·접선 기울기, 단항식 정적분을 다룬다. 카탈로그에 존재한다고 해서 즉시 모든 항목을 자동 풀이한다는 뜻은 아니며, 웹의 엔진 알고리즘 탭에서 이 차이를 공개한다.

최근 확장한 실행 규칙은 등비수열 일반항·합, 이항분포의 점확률, 평면벡터 성분 내적, 삼각함수 `sin`·`cos`의 미분값이다. 각 규칙은 입력 슬롯 부족, 확률 범위, 벡터 성분 누락을 PASS로 처리하지 않는다.
...
특정 시험 문항의 숫자와 결론을 고정한 전용 규칙은 사용하지 않는다. 실제 문항에서 발견된 지원 공백은 범용 계산 도구와 일반화된 입력 슬롯으로 확장하고, 지원할 수 없는 문항은 정답을 추정하지 않고 `FAIL`로 기록한다.
