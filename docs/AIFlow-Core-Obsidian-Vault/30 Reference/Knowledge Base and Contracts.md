...
- 수열: 등차수열, 등비수열 일반항·합
- 수학Ⅰ: 지수·로그(양의 유리수 밑의 일차 지수방정식, `n`제곱근·유리수 지수, 거듭제곱 관계 로그 밑의 2식 연립 비 포함), 특수각 삼각함수, 함수·합성·역함수
- 수학Ⅱ: 다항식 극한, 거듭제곱 미분·접선·정적분, 원점 동시 출발 두 점의 속도함수 위치 일치
- 미적분: `sin`, `cos` 미분값
...
`csg_3d_dot_product_angle`은 `vector_a`, `vector_b`의 정수 3성분을 `dot_product_3d`에 전달한다. 성분별 곱의 합을 양방향으로 다시 계산하고 각 벡터의 자기 내적 비음수성을 확인한다. 일반 `·`와 LaTeX `\cdot`가 정규화된 `*` 표기는 모두 같은 슬롯으로 수렴한다. 외적·각도·영벡터 각도는 이 계약 밖이다.
